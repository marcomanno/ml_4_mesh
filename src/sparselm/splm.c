 ...
 */
static MKL_INT splm_jacpatguess(void (*func)(double *p, double *hx, MKL_INT nvars, MKL_INT nobs, void *adata),
                             double *p, struct splm_ccsm *jac, MKL_INT nvars, MKL_INT nobs, void *adata,
  ...
  for(j=k=0; j<nvars; ++j){
    ...
    for(i=0; i<nobs; ++i){
      ...
      if(tmp>0.0){ // tmp>1E-07*d
        ...
        //printf("observation %Id depends on variable %Id [%.10lf]\n", i, j, tmp);
...
struct splm_fjacfuncs{
  void (*ccsfjac)(double *p, struct splm_ccsm *jac, MKL_INT nvars, MKL_INT nobs, void *adata);
  void (*crsfjac)(double *p, struct splm_crsm *jac, MKL_INT nvars, MKL_INT nobs, void *adata);
};
...
struct fdj_data_ {
  ...
  void (*func)(double *p, double *hx, MKL_INT nvars, MKL_INT nobs, void *adata); /* function to differentiate */
  MKL_INT forw;         /* 1: forward differences, 0: central differences */
  MKL_INT nfeval;       /* number of func evaluations for computing Jacobian */
  void *adata;
};
...
static void splm_fdif_jac(
    ...
    void *dat)
{
register MKL_INT i, j, jj, k;
register double d;
MKL_INT ii, m, *jcol, *varlist, *coldone, forw;
MKL_INT *vidxs, *ridxs;
double *tmpd;
...
double *hx, *hxx, delta;
  ...
  jcol=(MKL_INT *)emalloc(nobs*sizeof(MKL_INT)); /* keeps track of measurements influenced by the set of variables currently in "varlist" below */
  for(i=0; i<nobs; ++i) jcol[i]=-1;

  k=splm_ccsm_col_maxnelms(jac);
  vidxs=(MKL_INT *)emalloc(2*k*sizeof(MKL_INT));
  ridxs=vidxs+k;

  varlist=(MKL_INT *)emalloc(nvars*sizeof(MKL_INT)); /* stores indices of J's columns which are computed with the same "func" call */
  coldone=(MKL_INT *)emalloc(nvars*sizeof(MKL_INT)); /* keeps track of J's columns which have been already computed */
  memset(coldone, 0, nvars*sizeof(MKL_INT)); /* initialize to zero */
...
//printf("Jacobian for colunn %Id computed with %Id other columns\n", j, m-1);
...
//printf("Function evaluations for Jacobian computation: %Id\n", fdjd->nfeval);
}
 ...
 */
static double nrmL2xmy(double *e, const double *x, const double *y, const MKL_INT n)
{
 ...
 */
static double nrmL2my(double *e, const double *x, const double *y, const MKL_INT n)
{
const MKL_INT blocksize=8, bpwr=3; /* 8=2^3 */
register MKL_INT i;
MKL_INT blockn;
...
static MKL_INT splm_core(
    ...
    struct splm_fjacfuncs *const jacfs,       /* contains pointer to appropriate function for evaluating the sparse
                                               ...
                                               */
    MKL_INT jactyp,                               /* if equal to SPLM_ANJAC, jacfs->crsfjac/jacfs->ccsfjac computes
    ...
    const MKL_INT nvars,    /* I: parameter vector dimension (i.e. #unknowns) [m] */
    const MKL_INT nconvars, /* I: number of parameters (starting from the 1st) whose values should not be modified. >=0 */
    const MKL_INT nobs,     /* I: measurement vector dimension [n] */
    MKL_INT Jnnz,           /* I: number of nonzeros for the Jacobian J */
    MKL_INT JtJnnz,         /* I: number of nonzeros for the product J^t*J, -1 if unknown */
    const MKL_INT itmax,    /* I: maximum number of iterations */
...
const MKL_INT jacisccs=jacfs->ccsfjac || jactyp==SPLM_ZPJAC || jactyp==SPLM_NOJAC, /* is J in CCS? note that finite differentiation always uses CCS */
...
register MKL_INT itno;  /* iteration counter */
MKL_INT issolved;
...
double init_p_eL2;
MKL_INT nu=2, nu2, stop, nfev, njev=0, nlss=0;
double (*const L2xmy)(double *e, const double *x, const double *y, const MKL_INT n)=(x!=NULL)? nrmL2xmy : nrmL2my;
struct fdj_data_ fdj_data;
void *jac_adata;

/* direct sparse solver */
MKL_INT (*solver)(struct splm_ccsm *A, double *B, void **state, MKL_INT what, double *x)=NULL;
void *solvstate=NULL;
MKL_INT spsolver, hess_howto=2;
//double timea, timeb;

#ifdef USE_ATA_QUADS
MKL_INT nquads=0, quadsz[4];
void *quads[4]={NULL, NULL, NULL, NULL};
#endif

  /* error checking */
  if(nobs<nvars){
    fprintf(stderr, "splm_core(): cannot solve a problem with fewer measurements [%Id] than unknowns [%Id]\n", nobs, nvars);
    return SPLM_ERROR;
  }

  if(nconvars<0){
    fprintf(stderr, "splm_core(): number of variables to be kept constant must be non negative ([%Id])\n", nconvars);
    return SPLM_ERROR;
  }
  ...
  if(!jacfs->crsfjac && !jacfs->ccsfjac){
    ...
    fprintf(stderr, "splm_core(): invalid Jacobian size speficied, number of nonzero elements must be positive! [%Id]\n", Jnnz);
    return SPLM_ERROR;
  }
  ...
  switch(spsolver){
    ...
    default: {
      ...
      if(spsolver>0 && spsolver<sizeof(solvname)/sizeof(char *))
        ...
        fprintf(stderr, "\n*** unknown sparse direct solver \"%Id\" specified to splm_core(), exiting!\n\n", spsolver);
      exit(1);
    }
  }
...
//printf("%Id: %.2f\n", itno, timeb-timea);
      ...
      if(verbose && itno==0){
        printf("Jacobian and approximate Hessian nonzeros: %Id  %Id\n", jac_ccs.colptr[nvars], jacTjac.colptr[nvars-nconvars]);
   ...
   /* set up pointers to the diagonal of the Hessian & compute initial damping factor */
    if(itno==0){
      ...
      for(j=0; j<nvars-nconvars; ++j){ // CHECKME nconvars!
        ...
        if(ii>jj){
          fprintf(stderr, "splm_core(): Hessian's column %Id is empty, the corresponding Jacobian column is also empty?\n", j+nconvars);
          exit(1);
        }
        ...
        while(ii<=jj){
          ...
          i=((unsigned MKL_INT)ii + (unsigned MKL_INT)jj) >> 1; //(ii+jj)/2;
          ...
          fprintf(stderr, "splm_core() internal error: could not find the location corresponding to var %Id in the Hessian's diagonal\n", j+nconvars);
          exit(1);
        }
nextj:  continue;
      }
       ...
       if(dp_L2>=(p_L2+eps2)/SPLM_EPSILON_SQ){ /* almost singular */
        ...
        if(!SPLM_FINITE(pdp_eL2)){
          if(verbose) /* identify the offending prediction */
            for(i=0; i<nobs; ++i)
              if(!SPLM_FINITE(hx[i]))
                printf("sparseLM: component %Id of the predicted measurement vector is invalid!\n", i);
  ...
  if(info){
    ...
    info[5]= (double)itno;
    info[6]= (double)stop;
    info[7]= (double)(nfev + (jacisan? 0 : fdj_data.nfeval));
    info[8]= (double)njev;
    info[9]= (double)nlss;
  }
  ...
  if(verbose){
    ...
    fflush(stdout);
    fprintf(stdout, "sparseLM using %Id parameters (%Id constant) and %Id observations\n", nvars, nconvars, nobs);
    fprintf(stdout, "%s %s Jacobian, density %.1f%%\n\n", jacisan? "analytic" : "approximate", jacfs->ccsfjac? "CCS" : "CRS", density*100.0);
    fprintf(stdout, "sparseLM returning after %g iter, reason %g, error %g [initial %g], %Id/%Id func/fjac evals, %Id lin. systems\n", info[5], info[6], info[1], info[0], (MKL_INT)info[7], (MKL_INT)info[8], (MKL_INT)info[9]);
    fflush(stdout);
  }
...
MKL_INT sparselm_dercrs(void (*func)(double *p, double *hx, MKL_INT nvars, MKL_INT nobs, void *adata),
...
MKL_INT sparselm_derccs(void (*func)(double *p, double *hx, MKL_INT nvars, MKL_INT nobs, void *adata),
...
MKL_INT sparselm_difcrs(void (*func)(double *p, double *hx, MKL_INT nvars, MKL_INT nobs, void *adata),
                    void (*fjac)(double *p, struct splm_crsm *jac, MKL_INT nvars, MKL_INT nobs, void *adata),
                    double *p, double *x, MKL_INT nvars, const MKL_INT nconvars, MKL_INT nobs, MKL_INT Jnnz, MKL_INT JtJnnz,
                    MKL_INT itmax, double opts[SPLM_OPTS_SZ], double info[SPLM_INFO_SZ], void *adata)
{
...
MKL_INT sparselm_difccs(void (*func)(double *p, double *hx, MKL_INT nvars, MKL_INT nobs, void *adata),
                    void (*fjac)(double *p, struct splm_ccsm *jac, MKL_INT nvars, MKL_INT nobs, void *adata),
                    double *p, double *x, MKL_INT nvars, const MKL_INT nconvars, MKL_INT nobs, MKL_INT Jnnz, MKL_INT JtJnnz,
                    MKL_INT itmax, double opts[SPLM_OPTS_SZ], double info[SPLM_INFO_SZ], void *adata)
{
