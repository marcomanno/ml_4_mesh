...
void splm_crsm_alloc(struct splm_crsm *sm, MKL_INT nr, MKL_INT nc, MKL_INT nnz)
{
  sm->nr=nr;
  sm->nc=nc;
  sm->nnz=nnz;
  ...
  sm->colidx=(MKL_INT *)malloc(nnz*sizeof(MKL_INT));
  sm->rowptr=(MKL_INT *)malloc((nr+1)*sizeof(MKL_INT));
  if(!sm->val || !sm->colidx || !sm->rowptr){
    fprintf(stderr, "memory allocation request failed in splm_crsm_alloc() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
    exit(1);
  }
}
...
void splm_crsm_alloc_novalues(struct splm_crsm *sm, MKL_INT nr, MKL_INT nc, MKL_INT nnz)
{
  sm->nr=nr;
  sm->nc=nc;
  sm->nnz=nnz;
  ...
  if(!sm->colidx || !sm->rowptr){
    fprintf(stderr, "memory allocation request failed in splm_crsm_alloc_novalues() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
    exit(1);
  }
}
...
void splm_crsm_alloc_values(struct splm_crsm *sm)
{
  ...
  if(!sm->val){
    fprintf(stderr, "memory allocation request failed in splm_crsm_alloc_values() [nr=%Id, nc=%Id, nnz=%Id]\n", sm->nr, sm->nc, sm->nnz);
    exit(1);
  }
}

/* new dimensions & nonzero size, does not allocate values */
void splm_crsm_realloc_novalues(struct splm_crsm *sm, MKL_INT nr, MKL_INT nc, MKL_INT nnz)
{
  sm->nc=nc;
  sm->nnz=nnz;

  sm->colidx=(MKL_INT *)realloc(sm->colidx, nnz*sizeof(MKL_INT));
  if(!sm->colidx){
    fprintf(stderr, "memory allocation request failed in splm_crsm_realloc_novalues() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
    exit(1);
  }

  if(nr!=sm->nr){
    sm->nr=nr;
    sm->rowptr=(MKL_INT *)realloc(sm->rowptr, (nr+1)*sizeof(MKL_INT));
    if(!sm->rowptr){
      fprintf(stderr, "memory allocation request failed in splm_crsm_realloc() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
      exit(1);
    }
  }
}
...
static void splm_crsm_build(struct splm_crsm *sm, MKL_INT *m, MKL_INT nr, MKL_INT nc);
...
register MKL_INT i;
  ...
  fprintf(fp, "\ncolidx: ");
  for(i=0; i<sm->nnz; ++i)
    fprintf(fp, "%Id ", sm->colidx[i]);
  fprintf(fp, "\nrowptr: ");
  for(i=0; i<=sm->nr; ++i)
    fprintf(fp, "%Id ", sm->rowptr[i]);
  fprintf(fp, "\n");
}

/* build an MKL_INT sparse CRS matrix from a dense one. intended to serve as an example for sm creation */
static void splm_crsm_build(struct splm_crsm *sm, MKL_INT *m, MKL_INT nr, MKL_INT nc)
{
MKL_INT nnz;
...
MKL_INT splm_crsm_elmidx(struct splm_crsm *sm, MKL_INT i, MKL_INT j)
{
register MKL_INT low, high, mid, diff, *Colidx;
 ...
 */
MKL_INT splm_crsm_elmrow(struct splm_crsm *sm, MKL_INT idx)
{
register MKL_INT low, high, mid, diff, *Rowptr;
MKL_INT leq=-1;
 ...
 */
MKL_INT splm_crsm_row_elmidxs(struct splm_crsm *sm, MKL_INT i, MKL_INT *vidxs, MKL_INT *jidxs)
{
register MKL_INT j, k;
...
MKL_INT splm_crsm_row_maxnelms(struct splm_crsm *sm)
{
register MKL_INT i, n, max;
 ...
 */
MKL_INT splm_crsm_col_elmidxs(struct splm_crsm *sm, MKL_INT j, MKL_INT *vidxs, MKL_INT *iidxs)
{
register MKL_INT *nextrowptr=sm->rowptr+1;
register MKL_INT i, l;
...
MKL_INT *colcounts; // counters for the number of nonzeros in each column
  ...
  if((colcounts=(MKL_INT *)calloc(nc, sizeof(MKL_INT)))==NULL){ /* initialize to zero */
    fprintf(stderr, "memory allocation request failed in splm_crsm2ccsm() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
    exit(1);
  }
...
double *val, *tmpv;
  ...
  tmpi=(MKL_INT *)malloc(nc*sizeof(MKL_INT));
  if(!tmpv || !tmpi){
