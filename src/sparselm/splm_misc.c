...
void *splm_emalloc(char *file, MKL_INT line, size_t sz)
{
void *ptr;

  ptr=(void *)malloc(sz);
  if(ptr==NULL){
    fprintf(stderr, "memory allocation request for %zu bytes failed in file %s, line %Id, exiting", sz, file, line);
    exit(1);
  }
...
void *splm_realloc(char *file, MKL_INT line, void *oldptr, size_t sz)
{
void *ptr;

  ptr=(void *)realloc(oldptr, sz);
  if(ptr==NULL){
    fprintf(stderr, "memory re-allocation request for %zu bytes failed in file %s, line %Id, exiting\n", sz, file, line);
    exit(1);
  }
 ...
 */
static void splm_crcsm2eps(MKL_INT isCRS, void *sm, MKL_INT (*regions)[4], double (*colors)[3], MKL_INT nregions, char *fname)
{
struct splm_crsm *crs=NULL;
struct splm_ccsm *ccs=NULL;
register MKL_INT i, j, k;
MKL_INT nrows, ncols, brect=1;
MKL_INT dispw, disph; // size of displayed array in pts
FILE *fp;
time_t tim;
double scx, scy, scl;
  ...
  //if(brect) fprintf(fp, "%Id %Id %Id %Id rectstroke\n", _HORZ_OFFSET, _VERT_OFFSET, nrows, ncols);
...
void splm_crsm2eps(struct splm_crsm *sm, MKL_INT (*regions)[4], double (*colors)[3], MKL_INT nregions, char *fname)
{
  splm_crcsm2eps(1, (void *)sm, regions, colors, nregions, fname);
}

void splm_ccsm2eps(struct splm_ccsm *sm, MKL_INT (*regions)[4], double (*colors)[3], MKL_INT nregions, char *fname)
{
...
double epslog;
MKL_INT fvec_sz=n, pp_sz=m, fvecp_sz=n;
