...
struct solvstat{ // solver state
  ...
  /* 32-bit: MKL_INT pt[PSIZE]; 64-bit: long MKL_INT pt[PSIZE]      */
  /* or void *pt[PSIZE] should be OK on both architectures  */ 
  void *pt[PSIZE]; 
  /* Pardiso control parameters */
  MKL_INT iparm[PSIZE];
  MKL_INT n, mtype, nrhs, *ia, *ja, maxfct, mnum, msglvl;
  double *trivals;
};
 ...
 */
MKL_INT splm_Axb_PARDISO(struct splm_ccsm *A, double *B, void **state, MKL_INT what, double *x)
{
MKL_INT phase, trinnz, ret;
static MKL_INT num_procs=-1; // number of processors, determined on 1st call only

/* Auxiliary variables */
register MKL_INT i, j, k;
MKL_INT *rowptr, *colidx, error=0;
struct solvstat *stat;
  ...
  switch(what){
    ...
    case 1: /* initialize parameters, allocate memory & perform symbolic manipulation */
      *state=stat=(struct solvstat *)emalloc(sizeof(struct solvstat));
      ...
      pardisoinit(stat->pt, &(stat->mtype), stat->iparm); 
      ...
      /* set up indices */
      stat->ia=(MKL_INT *)emalloc((stat->n+1)*sizeof(MKL_INT));
      stat->ja=(MKL_INT *)emalloc(trinnz*sizeof(MKL_INT));
       ...
       */
      phase=11; 
      pardiso(stat->pt, &(stat->maxfct), &(stat->mnum), &(stat->mtype), &phase, &(stat->n), NULL /*trivals*/, stat->ia, stat->ja, NULL,
                    &(stat->nrhs), stat->iparm, &(stat->msglvl), NULL, NULL, &error);
      if(error!=0) {
        fprintf(stderr, "Pardiso error during symbolic factorization: %Id\n", error);
        //exit(1);
        ret=0;
        break;
      }
      ...
      //printf("\nNumber of nonzeros in factors  = %Id", stat->iparm[17]);
      //printf("\nNumber of factorization MFLOPS = %Id\n", stat->iparm[18]);
    ...
    case 2: /* numerical factorization & system solution */
      ...
      phase=22;
      pardiso(stat->pt, &(stat->maxfct), &(stat->mnum), &(stat->mtype), &phase, &(stat->n), stat->trivals, stat->ia, stat->ja, NULL,
                      &(stat->nrhs), stat->iparm, &(stat->msglvl), NULL, NULL, &error);
      if(error!=0) {
        fprintf(stderr, "Pardiso error during numerical factorization: %Id\n", error);
        //exit(2);
        ret=0;
        break;
      }
      ...
      if(error!=0) {
        fprintf(stderr, "Pardiso error during solution: %Id\n", error);
        //exit(3);
        ret=0;
      }
      else ret=1;
    ...
    default:
      fprintf(stderr, "Unknown mode %Id in splm_Axb_PARDISO()!\n", what);
      exit(1);
  }
  ...
  if(what==0 || what==3){
    ...
    pardiso(stat->pt, &(stat->maxfct), &(stat->mnum), &(stat->mtype), &phase, &(stat->n), NULL, stat->ia, stat->ja, NULL,
                    &(stat->nrhs), stat->iparm, &(stat->msglvl), NULL, NULL, &error);
    free(stat->ia);
    free(stat->ja);
    free(stat->trivals);
