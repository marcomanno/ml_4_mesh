...
void splm_ccsm_alloc(struct splm_ccsm *sm, MKL_INT nr, MKL_INT nc, MKL_INT nnz)
{
  sm->nr=nr;
  sm->nc=nc;
  sm->nnz=nnz;
  ...
  sm->rowidx=(MKL_INT *)malloc(nnz*sizeof(MKL_INT));
  sm->colptr=(MKL_INT *)malloc((nc+1)*sizeof(MKL_INT));
  if(!sm->val || !sm->rowidx || !sm->colptr){
    fprintf(stderr, "memory allocation request failed in splm_ccsm_alloc() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
    exit(1);
  }
}
...
void splm_ccsm_alloc_novalues(struct splm_ccsm *sm, MKL_INT nr, MKL_INT nc, MKL_INT nnz)
{
  sm->nr=nr;
  sm->nc=nc;
  sm->nnz=nnz;
  ...
  if(!sm->rowidx || !sm->colptr){
    fprintf(stderr, "memory allocation request failed in splm_ccsm_alloc_novalues() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
    exit(1);
  }
}
...
void splm_ccsm_alloc_values(struct splm_ccsm *sm)
{
  ...
  if(!sm->val){
    fprintf(stderr, "memory allocation request failed in splm_ccsm_alloc_values() [nr=%Id, nc=%Id, nnz=%Id]\n", sm->nr, sm->nc, sm->nnz);
    exit(1);
  }
}

/* new dimensions & nonzero size, does not allocate values */
void splm_ccsm_realloc_novalues(struct splm_ccsm *sm, MKL_INT nr, MKL_INT nc, MKL_INT nnz)
{
  sm->nr=nr;
  sm->nnz=nnz;

  sm->rowidx=(MKL_INT *)realloc(sm->rowidx, nnz*sizeof(MKL_INT));
  if(!sm->rowidx){
    fprintf(stderr, "memory allocation request failed in splm_ccsm_realloc_novalues() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
    exit(1);
  }

  if(nc!=sm->nc){
    sm->nc=nc;
    sm->colptr=(MKL_INT *)realloc(sm->colptr, (nc+1)*sizeof(MKL_INT));
    if(!sm->colptr){
      fprintf(stderr, "memory allocation request failed in splm_ccsm_realloc() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
      exit(1);
    }
  }
}
...
MKL_INT splm_ccsm_elmidx(struct splm_ccsm *sm, MKL_INT i, MKL_INT j)
{
register MKL_INT low, high, mid, *Rowidx, diff;
 ...
 */
MKL_INT splm_ccsm_elmcol(struct splm_ccsm *sm, MKL_INT idx)
{
register MKL_INT low, high, mid, diff, *Colptr;
MKL_INT leq=-1;
 ...
 */
MKL_INT splm_ccsm_row_elmidxs(struct splm_ccsm *sm, MKL_INT i, MKL_INT *vidxs, MKL_INT *jidxs)
{
register MKL_INT *nextcolptr=sm->colptr+1;
register MKL_INT j, l;
register MKL_INT low, high, mid, diff;
 ...
 */
MKL_INT splm_ccsm_col_elmidxs(struct splm_ccsm *sm, MKL_INT j, MKL_INT *vidxs, MKL_INT *iidxs)
{
register MKL_INT i, k;
MKL_INT low, high, *rowidx=sm->rowidx;
...
MKL_INT splm_ccsm_col_maxnelms(struct splm_ccsm *sm)
{
register MKL_INT j, n, max;
...
MKL_INT splm_ccsm_col_minnelms(struct splm_ccsm *sm)
{
register MKL_INT j, n, min;
...
static void splm_ccsm_build(struct splm_ccsm *sm, MKL_INT *m, MKL_INT nr, MKL_INT nc);
...
register MKL_INT i;
  ...
  fprintf(fp, "\nrowidx: ");
  for(i=0; i<sm->nnz; ++i)
    fprintf(fp, "%Id ", sm->rowidx[i]);
  fprintf(fp, "\ncolptr: ");
  for(i=0; i<=sm->nc; ++i)
    fprintf(fp, "%Id ", sm->colptr[i]);
  fprintf(fp, "\n");
}

/* build an MKL_INT sparse CCS matrix from a dense one. intended to serve as an example for sm creation */
static void splm_ccsm_build(struct splm_ccsm *sm, MKL_INT *m, MKL_INT nr, MKL_INT nc)
{
MKL_INT nnz;
 ...
 */
MKL_INT splm_ccsm_drop_cols(struct splm_ccsm *A, MKL_INT ncols)
{
register MKL_INT i;
MKL_INT ncnnz;
...
void splm_ccsm_restore_cols(struct splm_ccsm *A, MKL_INT ncols, MKL_INT ncnnz)
{
register MKL_INT i;
...
MKL_INT nr, nc, nnz;
MKL_INT *colidx, *rowptr, *rowidx, *colptr;
MKL_INT *rowcounts; // counters for the number of nonzeros in each row
  ...
  if((rowcounts=(MKL_INT *)calloc(nr, sizeof(MKL_INT)))==NULL){ /* initialize to zero */
    fprintf(stderr, "memory allocation request failed in splm_ccsm2crsm() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
    exit(1);
  }
...
double *val, *tmpv;
  ...
  tmpi=(MKL_INT *)malloc(nr*sizeof(MKL_INT));
  if(!tmpv || !tmpi){
