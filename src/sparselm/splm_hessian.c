...
#endif  /* USE_LINSRCH */
static MKL_INT binsearch_geq(const MKL_INT *mat, MKL_INT n, MKL_INT val)
{
register MKL_INT low=0, high=n, mid;
 ...
 */
void splm_setup_AtA_ccsA(struct splm_ccsm *A, struct splm_ccsm *AtA, const MKL_INT job)
{
register MKL_INT i, j, k;
register MKL_INT *AColptr, *ARowidx, *AtARowidx;
MKL_INT nc, *AtAColptr, *ARowidxM1, *AColptrP1;
MKL_INT ilow, ihigh, jlow, jhigh;
MKL_INT allocating=0;

#ifdef USE_BTA_MAP
unsigned char *rowmap; // bit array marking the nonzero rows of A's column j
register MKL_INT *ridx;
MKL_INT *lastp;
#endif
 ...
 */
void splm_calc_AtA_ccsA(struct splm_ccsm *A, struct splm_ccsm *AtA, const MKL_INT job)
{
register MKL_INT i, j, ii, jj, diff;
register MKL_INT *AColptr, *AColptrP1, *ARowidx;
MKL_INT *AtAColptr, *AtAColptrP1, *AtARowidx;
double *AVal, *AtAVal;
MKL_INT nc, idx, k;
register double sum;
...
{
#ifndef USE_MKL_ATX
register MKL_INT j, *ARowidx;
register double *AVal, sum0, sum1, sum2, sum3;
MKL_INT i, nc, *AColptr, *AColptrP1, jlow;
const MKL_INT blocksize=8, bpwr=3; /* 8=2^3 */
MKL_INT n, blockn;
...
register MKL_INT i;
double *AVal;
MKL_INT *AColptr, *ARowidx;

const double alpha=1.0, beta=0.0;
extern void mkl_dcscmv(char *transa, MKL_INT *m, MKL_INT *k, double *alpha, char *matdescra, double *val, MKL_INT *indx,
...
static MKL_INT compressVB_ndecr(MKL_INT uncompressed, MKL_INT *prev, unsigned char *compressed)
{
register MKL_INT delta, i=0;

  delta=uncompressed-*prev;
  if(delta<0){
    fprintf(stderr, "compressVB_ndecr(): cannot compress negative delta %Id\n", delta);
    exit(1);
  }
  *prev=uncompressed;
  while(delta>=128){
...
static MKL_INT uncompressVB_ndecr(const unsigned char *compressed, MKL_INT *prev, MKL_INT *uncompressed)
{
register MKL_INT p, shift, tmp, i=0;
...
static MKL_INT compressVB(MKL_INT uncompressed, MKL_INT *prev, unsigned char *compressed)
{
register MKL_INT delta, tmp, i=1;
...
static MKL_INT uncompressVB(const unsigned char *compressed, MKL_INT *prev, MKL_INT *uncompressed)
{
register MKL_INT shift, tmp, delta, i=1;
MKL_INT sg;
...
static MKL_INT compressVB_01(MKL_INT uncompressed, MKL_INT *prev, unsigned char *compressed)
{
register MKL_INT delta, i;
  ...
  *prev=uncompressed;
  ...
  if(delta==1){
    ...
    *compressed&=~(1<<i);
    ...
    fprintf(stderr, "invalid delta %Id in compressVB_01()!\n", delta);
    exit(1);
  }
  if(i<7) return 0;
...
/* previous element passed in prev[0], length n in prev[1] */
inline static MKL_INT uncompressVB_01(const unsigned char *compressed, MKL_INT *prev, MKL_INT *uncompressed)
{
register MKL_INT i=0;
...
/* max unsigned MKL_INT that can be represented with 28 bits */
 ...
 * elements.
 */
static MKL_INT compressS9_ndecr(MKL_INT idx, MKL_INT *prev, int32_t uncompressed[_UBUFSZ_S9NDEC], MKL_INT flush, uint32_t *compressed)
{
static MKL_INT codelen[]={28, 14, 9, 7, 5, 4, 3, 2 , 1 };
static MKL_INT codeno[] ={1 , 2 , 3, 4, 5, 7, 9, 14, 28};
register MKL_INT j, sel;
MKL_INT csz, cn, bl, delta;
uint32_t res, delta32, lim;
const MKL_INT codenosz=sizeof(codeno)/sizeof(MKL_INT);
/* circular buffer management */
MKL_INT start, end, nidx;
register MKL_INT cursor;
  ...
  if(!flush){
    ...
    if(end!=start){ // non full
      if((uint32_t)(delta) > (uint32_t)(__MAXUINT28)){ // delta<0 || delta>__MAXUINT28
        if(delta>0)
          fprintf(stderr, "compressS9_ndecr(): delta %Id cannot be compressed with 28 bits; use vByte compression\n", delta);
        else
          fprintf(stderr, "compressS9_ndecr(): cannot compress negative delta %Id\n", delta);
        exit(1);
      }
  ...
  while(nidx>0){
    for(sel=codenosz; sel-->0; ){
      ...
      if(j==0){
        //printf("%Id ints in 32 bits, code %Id\n", cn, sel);
        res|=sel<<28;
        compressed[csz++]=res;
        nidx-=cn;
...
/* uncompress a 32 bit MKL_INT encoding several indices according to the S-9 scheme.
 * Produces at most _MAXCOMP_S9NDEC elements
 */
static MKL_INT uncompressS9_ndecr(const uint32_t compressed, MKL_INT *prev, MKL_INT uncompressed[_MAXCOMP_S9NDEC])
{
register MKL_INT i, tmp;
MKL_INT j;
uint32_t sel;
 ...
 * elements.
 */
static MKL_INT compressS9(MKL_INT idx, MKL_INT *prev, int32_t uncompressed[_UBUFSZ_S9ARB], MKL_INT flush, uint32_t *compressed)
{
static MKL_INT codelen[] ={28, 14  , 9  , 7 , 5 , 4, 3, 2 };
static MKL_INT codeno[]  ={1 , 2   , 3  , 4 , 5 , 7, 9, 14};
static MKL_INT codeoffs[]={0 , 8191, 255, 63, 15, 7, 3, 1 }; /* 2^{codelen-1}-1, excluding codelen=28 */
register MKL_INT j, sel;
MKL_INT csz, cn, bl, delta;
  ...
  if(!flush){
    ...
    if(end!=start){ // non full
      if((uint32_t)(delta+__MAXUINT28) > (uint32_t)(__MAXUINT28<<1)){ // delta < -__MAXUINT28 || delta > __MAXUINT28
        fprintf(stderr, "compressS9(): delta %Id cannot be compressed with 28 bits; use vByte compression\n", delta);
        exit(1);
      }
      uncompressed[j]=delta;
    ...
    for(sel=codenosz; sel-->0; ){
      ...
      for(j=cn, cursor=start; j>0; --j){
        ...
        if(sel>0){
          ...
          if(tmpu>=lim) break; // (MKL_INT)tmpu<0 || tmpu>=lim
        }
        else{
          if(delta32>=0)
            tmpu=delta32;
...
/* uncompress a 32 bit MKL_INT encoding several indices according to a modified S-9 scheme.
 * Produces at most _MAXCOMP_S9ARB elements
 */
static MKL_INT uncompressS9(const uint32_t compressed, MKL_INT *prev, MKL_INT uncompressed[_MAXCOMP_S9ARB])
{
register MKL_INT i, tmp;
MKL_INT j;
uint32_t sel;
 ...
 */
MKL_INT splm_get_AtA_ccsA_quads(struct splm_ccsm *A, struct splm_ccsm *AtA, const MKL_INT job, void *quads[4], MKL_INT quadsz[4])
{
register MKL_INT i, j, ii, jj, diff;
register MKL_INT *AColptr, *AColptrP1, *ARowidx;
MKL_INT *AtAColptr, *AtAColptrP1, *AtARowidx;
MKL_INT nc, idx, k;
MKL_INT nquads=0, curlen=0;
#ifdef USE_VBYTE_COMPRESSION
unsigned char *lenptr, *iiptr, *jjptr, *kptr; // len, ii0, jj0, k
MKL_INT (*const compress_k)(MKL_INT, MKL_INT *, unsigned char *)=(job==1)? compressVB_01 : compressVB_ndecr;
//const MKL_INT eltsz=sizeof(unsigned char);
MKL_INT lastk[2]={0, 0};
#else
...
//const MKL_INT eltsz=sizeof(uint32_t);
MKL_INT lastk=0;
#endif /* USE_VBYTE_COMPRESSION */
MKL_INT lensz, iisz, jjsz, ksz; // allocated sizes for the above
MKL_INT lenN=0, iiN=0, jjN=0, kN=0; // actual lengths for the above
MKL_INT previi, prevjj, lastlen=0, lastii=0, lastjj=0; // note: previi, prevjj have a different role than lastii, lastjj!
MKL_INT totlen=0;
   ...
   *   a) sizeof(MKL_INT) bytes for a compressed integer when using vByte
   ...
   */
#ifdef USE_VBYTE_COMPRESSION
  lenptr=(unsigned char *)emalloc(lensz*sizeof(unsigned char)+sizeof(MKL_INT));
  iiptr =(unsigned char *)emalloc(iisz*sizeof(unsigned char)+sizeof(MKL_INT));
  jjptr =(unsigned char *)emalloc(jjsz*sizeof(unsigned char)+sizeof(MKL_INT));
  kptr  =(unsigned char *)emalloc(ksz*sizeof(unsigned char)+sizeof(MKL_INT));
#else
        ...
        if(diff==0){
          ...
          if(ii==previi+1 && jj==prevjj+1){
            ...
            if(unlikely(lenN>=lensz)){
              lensz<<=1; //lensz*=2;
#ifdef USE_VBYTE_COMPRESSION
              lenptr=(unsigned char *)realloc(lenptr, lensz*sizeof(unsigned char)+sizeof(MKL_INT));
#else
              lenptr=(uint32_t *)realloc(lenptr, (lensz+_MAXCOMPS9)*sizeof(uint32_t));
#endif
              if(lenptr==NULL){
                fprintf(stderr, "memory reallocation request for %Id bytes failed in splm_get_AtA_ccsA_quads() [len]\n", lensz);
                exit(1);
              }
            }
            if(unlikely(iiN>=iisz)){
              iisz<<=1; //iisz*=2;
#ifdef USE_VBYTE_COMPRESSION
              iiptr=(unsigned char *)realloc(iiptr, iisz*sizeof(unsigned char)+sizeof(MKL_INT));
#else
              iiptr=(uint32_t *)realloc(iiptr, (iisz+_MAXCOMPS9)*sizeof(uint32_t));
#endif
              if(iiptr==NULL){
                fprintf(stderr, "memory reallocation request for %Id bytes failed in splm_get_AtA_ccsA_quads() [i]\n", iisz);
                exit(1);
              }
            }
            if(unlikely(jjN>=jjsz)){
              jjsz<<=1; //jjsz*=2;
#ifdef USE_VBYTE_COMPRESSION
              jjptr=(unsigned char *)realloc(jjptr, jjsz*sizeof(unsigned char)+sizeof(MKL_INT));
#else
              jjptr=(uint32_t *)realloc(jjptr, (jjsz+_MAXCOMPS9)*sizeof(uint32_t));
#endif
              if(jjptr==NULL){
                fprintf(stderr, "memory reallocation request for %Id bytes failed in splm_get_AtA_ccsA_quads() [j]\n", jjsz);
                exit(1);
              }
            }
            if(unlikely(kN>=ksz)){
              ksz<<=1; //ksz*=2;
#ifdef USE_VBYTE_COMPRESSION
              kptr=(unsigned char *)realloc(kptr, ksz*sizeof(unsigned char)+sizeof(MKL_INT));
#else
              kptr=(uint32_t *)realloc(kptr, (ksz+_MAXCOMPS9)*sizeof(uint32_t));
#endif
              if(kptr==NULL){
                fprintf(stderr, "memory reallocation request for %Id bytes failed in splm_get_AtA_ccsA_quads() [k]\n", ksz);
                exit(1);
              }
            }
...
#if 0
  fprintf(stderr, "splm_get_AtA_ccsA_quads(): found %Id quads, avg. length %.2lf, %s compressed size %.2lf Mb, uncompressed %.2lf\n",
          ...
          "S9",
#endif
          (lenN+iiN+jjN+kN)*eltsz/1048576.0, nquads*sizeof(MKL_INT)*4/1048576.0);
#endif
...
void splm_calc_AtA_ccsA_quads(struct splm_ccsm *A, struct splm_ccsm *AtA, const MKL_INT job, void *quads[4], MKL_INT nquads)
{
register MKL_INT i, j;
MKL_INT j0, ii0, jj0, k;
...
MKL_INT (*const uncompress_k)(const unsigned char *, MKL_INT *, MKL_INT *)=(job==1)? uncompressVB_01 : uncompressVB_ndecr;
MKL_INT lastk[2]={0, 0};
#else
const uint32_t *lenptr, *iiptr, *jjptr, *kptr; // len, ii0, jj0, k
MKL_INT lenbuf[_MAXCOMPS9], iibuf[_MAXCOMPS9], jjbuf[_MAXCOMPS9], kbuf[_MAXCOMPS9];
MKL_INT lenbufsz=0, iibufsz=0, jjbufsz=0, kbufsz=0;
MKL_INT ilen=_MAXCOMPS9, iii=_MAXCOMPS9, ijj=_MAXCOMPS9, ik=_MAXCOMPS9;
MKL_INT lastk=0;
#endif /* USE_VBYTE_COMPRESSION */
MKL_INT lenN=0, iiN=0, jjN=0, kN=0;
MKL_INT lastlen=0, lastii=0, lastjj=0;
...
MKL_INT splm_get_AtA_crsA_quads(struct splm_crsm *A, MKL_INT zerocols, struct splm_ccsm *AtA, const MKL_INT job, void *quads[4], MKL_INT quadsz[4])
{
MKL_INT nr, idx=0; /* -Wall */
register MKL_INT i, j, k, r1, r2;
register MKL_INT low, high;
MKL_INT *ARowptr, *ARowptrP1, *AColidx, *AtAColptr, *AtAColptrP1, *AtARowidx, *AtARowidxP1;
MKL_INT nquads=0, curlen=0;
#ifdef USE_VBYTE_COMPRESSION
...
//const MKL_INT eltsz=sizeof(uint32_t);
#endif /* USE_VBYTE_COMPRESSION */
MKL_INT lensz, r1sz, r2sz, idxsz; // allocated sizes for the above
MKL_INT lenN=0, r1N=0, r2N=0, idxN=0; // actual lengths for the above
MKL_INT previdx, prevr2, lastlen=0, lastr1=0, lastr2=0, lastidx=0; // note: previdx, prevr2 have a different role than lastidx, lastr2!
MKL_INT totlen=0;
...
#ifdef USE_VBYTE_COMPRESSION
  lenptr=(unsigned char *)emalloc(lensz*sizeof(unsigned char)+sizeof(MKL_INT));
  r1ptr =(unsigned char *)emalloc(r1sz*sizeof(unsigned char)+sizeof(MKL_INT));
  r2ptr =(unsigned char *)emalloc(r2sz*sizeof(unsigned char)+sizeof(MKL_INT));
  idxptr=(unsigned char *)emalloc(idxsz*sizeof(unsigned char)+sizeof(MKL_INT));
#else
  lenptr=(uint32_t *)emalloc((lensz+_MAXCOMPS9)*sizeof(uint32_t));
      ...
      for(r2=binsearch_geq(AColidx+r1, ARowptrP1[k]-r1, zerocols) + r1, // ignore elements whose colidx < zerocols
        ...
        if(idx<=0 || AtARowidxP1[idx]!=i){
          ...
          if(high==AtAColptrP1[j] || AtARowidx[high]!=i){
            ...
            fprintf(stderr, "splm_get_AtA_crsA_quads() internal error: no element in column %Id of A^t*A "
                            "with row index equal to %Id found!\n\tmalformed Jacobian?\n", i+zerocols, j+zerocols);
            exit(1);
          }
#endif
        }
        ...
        if(idx==previdx+1 && r2==prevr2+1){
...
#endif
            if(lenptr==NULL){
              fprintf(stderr, "memory reallocation request for %Id bytes failed in splm_get_AtA_crsA_quads() [len]\n", lensz);
              exit(1);
            }
          }
          if(unlikely(r1N>=r1sz)){
            r1sz<<=1; //r1sz*=2;
#ifdef USE_VBYTE_COMPRESSION
            r1ptr=(unsigned char *)realloc(r1ptr, r1sz*sizeof(unsigned char)+sizeof(MKL_INT));
#else
            r1ptr=(uint32_t *)realloc(r1ptr, (r1sz+_MAXCOMPS9)*sizeof(uint32_t));
#endif
            if(r1ptr==NULL){
              fprintf(stderr, "memory reallocation request for %Id bytes failed in splm_get_AtA_crsA_quads() [r1]\n", r1sz);
              exit(1);
            }
          }
          if(unlikely(r2N>=r2sz)){
            r2sz<<=1; //r2sz*=2;
#ifdef USE_VBYTE_COMPRESSION
            r2ptr=(unsigned char *)realloc(r2ptr, r2sz*sizeof(unsigned char)+sizeof(MKL_INT));
#else
            r2ptr=(uint32_t *)realloc(r2ptr, (r2sz+_MAXCOMPS9)*sizeof(uint32_t));
#endif
            if(r2ptr==NULL){
              fprintf(stderr, "memory reallocation request for %Id bytes failed in splm_get_AtA_crsA_quads() [r2]\n", r2sz);
              exit(1);
            }
          }
          if(unlikely(idxN>=idxsz)){
            idxsz<<=1; //idxsz*=2;
#ifdef USE_VBYTE_COMPRESSION
            idxptr=(unsigned char *)realloc(idxptr, idxsz*sizeof(unsigned char)+sizeof(MKL_INT));
#else
            idxptr=(uint32_t *)realloc(idxptr, (idxsz+_MAXCOMPS9)*sizeof(uint32_t));
#endif
            if(idxptr==NULL){
              fprintf(stderr, "memory reallocation request for %Id bytes failed in splm_get_AtA_crsA_quads() [idx]\n", idxsz);
              exit(1);
            }
          }
...
#if 0
  fprintf(stderr, "splm_get_AtA_crsA_quads(): found %Id quads, avg. length %.2lf, %s compressed size %.2lf Mb, uncompressed %.2lf\n",
          nquads, totlen/(double)nquads,
#ifdef USE_VBYTE_COMPRESSION
          "vByte",
#else
          "S9",
#endif
          (lenN+r1N+r2N+idxN)*eltsz/1048576.0, nquads*sizeof(MKL_INT)*4/1048576.0);
#endif
...
void splm_calc_AtA_crsA_quads(struct splm_crsm *A, MKL_INT zerocols, struct splm_ccsm *AtA, const MKL_INT job, void *quads[4], MKL_INT nquads)
{
register MKL_INT i, j;
MKL_INT j0, r1, r2, idx, nc;
double *AVal, *AtAVal;
#ifdef USE_VBYTE_COMPRESSION
const unsigned char *lenptr, *r1ptr, *r2ptr, *idxptr; // len, r1, r2, idx
#else
const uint32_t *lenptr, *r1ptr, *r2ptr, *idxptr; // len, r1, r2, idx
MKL_INT lenbuf[_MAXCOMPS9], r1buf[_MAXCOMPS9], r2buf[_MAXCOMPS9], idxbuf[_MAXCOMPS9];
MKL_INT lenbufsz=0, r1bufsz=0, r2bufsz=0, idxbufsz=0;
MKL_INT ilen=_MAXCOMPS9, ir1=_MAXCOMPS9, ir2=_MAXCOMPS9, iidx=_MAXCOMPS9;
#endif /* USE_VBYTE_COMPRESSION */
MKL_INT lenN=0, r1N=0, r2N=0, idxN=0;
MKL_INT lastlen=0, lastr1=0, lastr2=0, lastidx=0;
 ...
 */
void splm_calc_Atx_crsA(struct splm_crsm *A, MKL_INT zerocols, double *x, double *y)
{
register MKL_INT k, r1;
MKL_INT j, nr, nc, *ARowptr, *ARowptrP1, *AColidx;
double *AVal;
register double xk;
 ...
 * If job is > 1, computes the structure of the full matrix product (i.e., all L, D & U parts)
 */
void splm_setup_AtA_crsA(struct splm_crsm *A, MKL_INT zerocols, struct splm_ccsm *AtA, const MKL_INT job)
{
register MKL_INT i, j, k, jj;
register MKL_INT *AtRowptr, *AColidx;
MKL_INT *ARowptr, *AtColidx, *ARowptrP1;
MKL_INT nr, nc, nnz;
...
struct splm_ccsm ccsA;
  ...
  colcounts=(MKL_INT *)calloc(nc, sizeof(MKL_INT)); /* init to zero */
  AtRowptr=(MKL_INT *)malloc((nc+1+nnz)*sizeof(MKL_INT));
  if(!colcounts || !AtRowptr){
    fprintf(stderr, "memory allocation request failed in splm_setup_AtA_crsA() [nr=%Id, nc=%Id, nnz=%Id]\n", nr, nc, nnz);
    exit(1);
  }
 ...
 *
 */
void splm_calc_AtAx_crsA(struct splm_crsm *A, MKL_INT zerocols, struct splm_ccsm *AtA, const MKL_INT job, double *const x, double *const y)
{
MKL_INT nr, nc, idx=0; /* -Wall */
register MKL_INT i, j, k, r1, r2;
register MKL_INT low, high;
double *AVal, *AtAVal;
    ...
    for(r1=binsearch_geq(AColidx+ARowptr[k], ARowptrP1[k]-ARowptr[k], zerocols) + ARowptr[k]; // ignore elements whose colidx < zerocols
      ...
      for(r2=binsearch_geq(AColidx+r1, ARowptrP1[k]-r1, zerocols) + r1, // ignore elements whose colidx < zerocols
        ...
        if(idx<=0 || AtARowidxP1[idx]!=i){
          ...
          if(high==AtAColptrP1[j] || AtARowidx[high]!=i){
            /* should not reach this point... */
            fprintf(stderr, "splm_calc_AtAx_crsA() internal error: no element in column %Id of A^t*A "
                            "with row index equal to %Id found!\n\tmalformed Jacobian?\n", i+zerocols, j+zerocols);
            exit(1);
          }
#endif
